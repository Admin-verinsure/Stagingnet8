...
{
    public class MREMLUWModule : IUnderwritingModule
    {
        public string Name { get; protected set; }

        public MREMLUWModule()
        {
            Name = "MRE_ML";
        }
        ...
        public bool Underwrite(User underwritingUser, ClientInformationSheet informationSheet, Product product, string reference)
        {
            ...
            if (agreement.ClientAgreementTerms.Where(ct => ct.SubTermType == "ML" && ct.DateDeleted == null) != null)
            {
                foreach (ClientAgreementTerm mlterm in agreement.ClientAgreementTerms.Where(ct => ct.SubTermType == "ML" && ct.DateDeleted == null))
                {
                    mlterm.Delete(underwritingUser);
                }
            }
            ...
            if (agreement.ClientInformationSheet.PreRenewOrRefDatas.Count() > 0)
            {
                foreach (var preRenewOrRefData in agreement.ClientInformationSheet.PreRenewOrRefDatas)
                {
                    ...
                    {
                        if (!string.IsNullOrEmpty(preRenewOrRefData.MLRetro))
                        {
                            strretrodate = preRenewOrRefData.MLRetro;
                        }

                    }
                    if (preRenewOrRefData.DataType == "preendorsement" && preRenewOrRefData.EndorsementProduct == "ML")
                    {
            ...
            //{
            //    var renewFromAgreement = agreement.ClientInformationSheet.RenewFromInformationSheet.Programme.Agreements.FirstOrDefault(p => p.ClientAgreementTerms.Any(i => i.SubTermType == "ML"));
            ...
            if (agreement.Product.IsOptionalProduct && agreement.ClientInformationSheet.Answers.Where(sa => sa.ItemName == agreement.Product.OptionalProductRequiredAnswer).First().Value == "1" &&
                agreement.ClientInformationSheet.Answers.Where(sa => sa.ItemName == "MLViewModel.HasInsolvencyOptions").First().Value == "1")
            {
                if (cAEDOInsExcl != null)
                {
            ...
            int TermLimit = 1000000;
            decimal TermPremium = 0M;
            decimal TermBasePremium = 0M;
            decimal TermBrokerage = 0M;
            decimal TermExcess = 5000;

            TermPremium = TermPremium * agreementperiodindays / coverperiodindays;
            TermBrokerage = TermPremium * agreement.Brokerage / 100;

            ClientAgreementTerm termmltermoption = GetAgreementTerm(underwritingUser, agreement, "ML", TermLimit, TermExcess);
            termmltermoption.TermLimit = TermLimit;
            termmltermoption.Premium = TermPremium;
            termmltermoption.BasePremium = TermBasePremium;
            termmltermoption.Excess = TermExcess;
            termmltermoption.BrokerageRate = agreement.Brokerage;
            termmltermoption.Brokerage = TermBrokerage;
            termmltermoption.DateDeleted = null;
            termmltermoption.DeletedBy = null;
            ...
            if (agreement.ClientInformationSheet.IsChange && agreement.ClientInformationSheet.PreviousInformationSheet != null)
            {
                ...
                var PreviousAgreement = agreement.ClientInformationSheet.PreviousInformationSheet.Programme.Agreements.FirstOrDefault(p => p.ClientAgreementTerms.Any(i => i.SubTermType == "ML"));
                foreach (var term in PreviousAgreement.ClientAgreementTerms)
                {
                    if (term.Bound)
                    {
                        ...
                        termmltermoption.PremiumDiffer = (TermPremium - PreviousBoundPremium) * coverperiodindaysforchange / agreementperiodindays;
                        termmltermoption.PremiumPre = PreviousBoundPremium;
                        if (termmltermoption.TermLimit == term.TermLimit && termmltermoption.Excess == term.Excess)
                        {
                            termmltermoption.Bound = true;
                        }
                    }
            ...
            string auditLogDetail = "MRE ML UW created/modified";
