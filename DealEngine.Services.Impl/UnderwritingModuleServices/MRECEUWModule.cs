...
{
    public class MRECEUWModule : IUnderwritingModule
    {
        public string Name { get; protected set; }

        public MRECEUWModule()
        {
            Name = "MRE_CE";
        }
        ...
        public bool Underwrite(User underwritingUser, ClientInformationSheet informationSheet, Product product, string reference)
        {
            ...
            if (agreement.ClientAgreementTerms.Where(ct => ct.SubTermType == "CE" && ct.DateDeleted == null) != null)
            {
                foreach (ClientAgreementTerm ceterm in agreement.ClientAgreementTerms.Where(ct => ct.SubTermType == "CE" && ct.DateDeleted == null))
                {
                    ceterm.Delete(underwritingUser);
                }
            }
            ...
            if (agreement.ClientInformationSheet.PreRenewOrRefDatas.Count() > 0)
            {
                foreach (var preRenewOrRefData in agreement.ClientInformationSheet.PreRenewOrRefDatas)
                {
                    ...
                    {
                        if (!string.IsNullOrEmpty(preRenewOrRefData.CERetro))
                        {
                            strretrodate = preRenewOrRefData.CERetro;
                        }

                    }
                    if (preRenewOrRefData.DataType == "preendorsement" && preRenewOrRefData.EndorsementProduct == "CE")
                    {
            ...
            //{
            //    var renewFromAgreement = agreement.ClientInformationSheet.RenewFromInformationSheet.Programme.Agreements.FirstOrDefault(p => p.ClientAgreementTerms.Any(i => i.SubTermType == "CE"));
            ...
            ClientAgreementTerm termcetermoption = GetAgreementTerm(underwritingUser, agreement, "CE", TermLimit, TermExcess);
            termcetermoption.TermLimit = TermLimit;
            termcetermoption.Premium = TermPremium;
            termcetermoption.BasePremium = TermBasePremium;
            termcetermoption.Excess = TermExcess;
            termcetermoption.BrokerageRate = agreement.Brokerage;
            termcetermoption.Brokerage = TermBrokerage;
            termcetermoption.DateDeleted = null;
            termcetermoption.DeletedBy = null;
            ...
            if (agreement.ClientInformationSheet.IsChange && agreement.ClientInformationSheet.PreviousInformationSheet != null)
            {
                ...
                var PreviousAgreement = agreement.ClientInformationSheet.PreviousInformationSheet.Programme.Agreements.FirstOrDefault(p => p.ClientAgreementTerms.Any(i => i.SubTermType == "CE"));
                foreach (var term in PreviousAgreement.ClientAgreementTerms)
                {
                    if (term.Bound)
                    {
                        ...
                        termcetermoption.PremiumDiffer = (TermPremium - PreviousBoundPremium) * coverperiodindaysforchange / agreementperiodindays;
                        termcetermoption.PremiumPre = PreviousBoundPremium;
                        if (termcetermoption.TermLimit == term.TermLimit && termcetermoption.Excess == term.Excess)
                        {
                            termcetermoption.Bound = true;
                        }
                    }
            ...
            string auditLogDetail = "MRE CE UW created/modified";
