...
{
    public class NZBarILUWModule : IUnderwritingModule
    {
        public string Name { get; protected set; }

        public NZBarILUWModule()
        {
            Name = "NZBar_IL";
        }
        ...
        public bool Underwrite(User underwritingUser, ClientInformationSheet informationSheet, Product product, string reference)
        {
            ...
            if (agreement.ClientAgreementTerms.Where(ct => ct.SubTermType == "IL" && ct.DateDeleted == null) != null)
            {
                foreach (ClientAgreementTerm ilterm in agreement.ClientAgreementTerms.Where(ct => ct.SubTermType == "IL" && ct.DateDeleted == null))
                {
                    ilterm.Delete(underwritingUser);
                }
            }

            IDictionary<string, decimal> rates = BuildRulesTable(agreement, "iltermlimit", "iltermexcess", "iltermpremium");
            ...
            if (agreement.ClientInformationSheet.PreRenewOrRefDatas.Count() > 0)
            {
                foreach (var preRenewOrRefData in agreement.ClientInformationSheet.PreRenewOrRefDatas)
                {
                    ...
                    {
                        if (!string.IsNullOrEmpty(preRenewOrRefData.ILRetro))
                        {
                            strretrodate = preRenewOrRefData.ILRetro;
                        }

                    }
                    if (preRenewOrRefData.DataType == "preendorsement" && preRenewOrRefData.EndorsementProduct == "IL")
                    {
            ...
            //{
            //    var renewFromAgreement = agreement.ClientInformationSheet.RenewFromInformationSheet.Programme.Agreements.FirstOrDefault(p => p.ClientAgreementTerms.Any(i => i.SubTermType == "IL"));
            ...
            TermLimit = Convert.ToInt32(rates["iltermlimit"]);
            TermExcess = rates["iltermexcess"];
            TermPremium = rates["iltermpremium"];
            ...
            ClientAgreementTerm termiltermoption = GetAgreementTerm(underwritingUser, agreement, "IL", TermLimit, TermExcess);
            termiltermoption.TermLimit = TermLimit;
            termiltermoption.Premium = TermPremium;
            termiltermoption.BasePremium = TermBasePremium;
            termiltermoption.Excess = TermExcess;
            termiltermoption.BrokerageRate = agreement.Brokerage;
            termiltermoption.Brokerage = TermBrokerage;
            termiltermoption.DateDeleted = null;
            termiltermoption.DeletedBy = null;
            ...
            if (agreement.ClientInformationSheet.IsChange && agreement.ClientInformationSheet.PreviousInformationSheet != null)
            {
                var PreviousAgreement = agreement.ClientInformationSheet.PreviousInformationSheet.Programme.Agreements.Where(a => a.DateDeleted == null).
                    FirstOrDefault(p => p.ClientAgreementTerms.Any(i => i.SubTermType == "IL"));
                foreach (var term in PreviousAgreement.ClientAgreementTerms)
                {
                    if (term.Bound)
                    {
                        ...
                        termiltermoption.PremiumDiffer = (TermPremium - PreviousBoundPremium) * coverperiodindaysforchange / agreementperiodindays;
                        termiltermoption.PremiumPre = PreviousBoundPremium;
            ...
            string auditLogDetail = "NZBar IL UW created/modified";
